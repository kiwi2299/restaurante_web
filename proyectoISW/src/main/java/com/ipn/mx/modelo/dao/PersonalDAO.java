...
public class PersonalDAO {
    ...
    public int crear(PersonalDTO dto) {
        Session sesion = HibernateUtil.getSessionFactory().getCurrentSession();
        Transaction transaccion = sesion.getTransaction();
        int i = 0;
        try {
            transaccion.begin();
            i = (int) sesion.save(dto.getEntidad());
    ...
    public void delete(PersonalDTO dto) {
        Session sesion = HibernateUtil.getSessionFactory().getCurrentSession();
        Transaction transaccion = sesion.getTransaction();
        try {
            transaccion.begin();
    ...
    public PersonalDTO read(PersonalDTO dto) {
        Session sesion = HibernateUtil.getSessionFactory().getCurrentSession();
        Transaction transaccion = sesion.getTransaction();
        try {
            transaccion.begin();
            dto.setEntidad(sesion.get(dto.getEntidad().getClass(), dto.getEntidad().getIdPersonal()));
            transaccion.commit();
            return dto;
        } catch (HibernateException he) {
            if (transaccion != null && transaccion.isActive()) {
    ...
    public List readAll() {
        ...
        try {
            transaccion.begin();
            Query q = sesion.createQuery("from Personal p order by p.idPersonal");
            for (Personal d : (List<Personal>) q.list()) {
